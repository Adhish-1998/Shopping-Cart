...
const createProduct = async (req, res) => {
    try {
        ...
        if (currencyFormat || currencyFormat == '') {
            if (!isValidBody(currencyFormat)) return res.status(400).send({ status: false, message: "Please enter currencyFormat !!!" });
        ...
        if (isFreeShipping || isFreeShipping == '') {
            if (!isValidBody(isFreeShipping)) return res.status(400).send({ status: false, message: "Please enter value of Free Shipping !!!" });
        ...
        if (file.length == 0) return res.status(400).send({ status: false, message: "Please Enter Product Image" })
        ...
        if (!isValidBody(availableSizes)) return res.status(400).send({ status: false, message: "Please enter Size !!!" });
        availableSizes = availableSizes.split(',').map((item) => item.trim())
        for (let i = 0; i < availableSizes.length; i++) {
            if (!sizes.includes(availableSizes[i]))
                return res.status(400).send({ status: false, message: "Please mention valid Size !!!" });
        }
        obj.availableSizes = availableSizes
        if (!isValidBody(installments)) return res.status(400).send({ status: false, message: "Please enter installments !!!" });
        if (!(/^[0-9]+$/.test(installments))) return res.status(400).send({ status: false, message: "Indstallments must be number!!!" });
        ...
        if (findTitle) {
            return res.status(400).send({ status: false, message: "Title Is Already Exists, Plz Enter Another One !!!" });
        }
...
const getProduct = async (req, res) => {
    ...
    if (size || size == '') {
        ...
        for (let i = 0; i < size.length; i++) {
            if (!sizes.includes(size[i]))
                return res.status(400).send({ status: false, message: "Please mention valid Size In Body !!!" });
        }
    ...
    if (name || name == '') {
        ...
        if (!validProdName(name)) return res.status(400).send({ status: false, message: "Please mention valid name !!!" });
    ...
    if (priceGreaterThan || priceGreaterThan == '') {
        ...
        if (!isValidPrice(priceGreaterThan)) return res.status(400).send({ status: false, message: "Price Greater Than must be number !!!" });
    ...
    if (priceLessThan || priceLessThan == '') {
        ...
        if (!isValidPrice(priceLessThan)) return res.status(400).send({ status: false, message: "Price Lesser Than must be number !!!" });
...
const updateProduct = async (req, res) => {
    ...
    if (Object.keys(req.body).length == 0)
        return res.status(400).send({ status: false, message: "Minimum 1 field required to be updated !!!" });
    ...
    if (price || price == '') {
        if (!isValidBody(price)) return res.status(400).send({ status: false, message: "Please enter price !!!" });
        if (!isValidPrice(price)) return res.status(400).send({ status: false, message: "Price Must be Number!!!" });
        UpObj.price = price
    }
    ...
    if (isFreeShipping || isFreeShipping == '') {
        if (!isValidBody(isFreeShipping)) return res.status(400).send({ status: false, message: "Please enter value of Free Shipping !!!" });
        if (isFreeShipping !== 'true') return res.status(400).send({ status: false, message: "Please valid value of Free shipping !!!" });
        UpObj.isFreeShipping = isFreeShipping
    }
    ...
    if (style || style == '') {
        if (!isValidBody(style)) return res.status(400).send({ status: false, message: "Please enter style !!!" });
        if (!isValidName(style)) return res.status(400).send({ status: false, message: "Please mention valid style In Body !!!" });
    ...
    if (installments || installments == '') {
        if (!isValidBody(installments)) return res.status(400).send({ status: false, message: "Please enter installments !!!" });
        if (!(/^[0-9]+$/.test(installments))) return res.status(400).send({ status: false, message: "Please mention valid indstallments !!!" });
...
const deleteById = async function (req, res) {
    try {
        ...
        if (productId == ':productId') return res.status(400).send({ status: false, message: "Please Enter Product Id" })
